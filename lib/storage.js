...
//   ai_gen   —  (id SERIAL, user_id TEXT, case_ids TEXT, mode TEXT, result_text TEXT, created_at TIMESTAMPTZ DEFAULT NOW())
...
async function _init() {
  ...
  await sql`CREATE TABLE IF NOT EXISTS ai_gen (id SERIAL PRIMARY KEY, user_id TEXT, case_ids TEXT, mode TEXT, result_text TEXT, created_at TIMESTAMPTZ DEFAULT NOW())`;
  _initDone = true;
}
...
async function addAIGen(record) {
  await _init();
  await sql`
    INSERT INTO ai_gen (user_id, case_ids, mode, result_text) VALUES (${record.user_id || null}, ${record.case_ids || ''}, ${record.mode || 'standard'}, ${record.result_text || ''})
  `;
  // 最多保留 500 条
...
async function getAllAIGen() {
  await _init();
  const rows = await sql`SELECT id, user_id, case_ids, mode, result_text, created_at FROM ai_gen ORDER BY id DESC LIMIT 500`;
  return rows.map(r => ({
    id: r.id,
    user_id: r.user_id,
    case_ids: r.case_ids,
