...
async function getAllClicks() {
  // 返回扁平格式 { case_id: count }，保持与前端 Math.max 合并逻辑兼容
  await _init();
  const rows = await sql`SELECT case_id, count FROM clicks`;
  const obj = {};
  for (const r of rows) obj[r.case_id] = r.count;
  return obj;
}

async function getAllClicksWithMeta() {
  // 管理员后台用：返回 { case_id: { count, last_clicked_at } }
  await _init();
